...
if (muteBtn && heroVideo) {
    muteBtn.addEventListener('click', () => {
        if (heroVideo.muted) {
            ...
            muteIcon.textContent = i18n[currentLang].mute_on; // '소리 끄기' 또는 'Mute'
            muteBtn.style.borderColor = 'var(--secondary)'; // 켜지면 초록색 테두리
        } else {
            heroVideo.muted = true;
            muteIcon.textContent = i18n[currentLang].mute_off; // '소리 켜기' 또는 'Unmute'
...
if (bSlider && hOverlay && hContent) {
    ...
    bSlider.addEventListener('input', (e) => {
        ...
        // 밝게 조절할수록 오버레이의 투명도는 낮아져서 비디오가 선명하게 보임
        const overlayOpacity = 1 - (val / 100);
        
        // 밝게 조절할수록 텍스트의 투명도가 0에 가까워짐 (점점 희미하게)
